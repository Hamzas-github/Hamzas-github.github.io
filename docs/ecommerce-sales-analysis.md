...
> **End-to-end analytics:** raw Excel to cleaned SQLite database to SQL analysis to
...
```
raw.xlsx  to  clean & validate (pandas)  to  SQLite DB  to  SQL analysis  to  charts  to  Power BI
```

1. **Data cleaning (pandas).** ~1.07M raw rows to **1.0M** clean sales lines. I split
 ...
 only positive quantity and price, and derived `revenue = quantity x price`.
