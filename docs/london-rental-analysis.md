...
> **End-to-end analytics:** raw CSV to cleaned SQLite database to SQL analysis to
> publication-quality Python visualizations to Power BI dashboard.
...
```
raw CSV  to  clean & validate (pandas)  to  SQLite DB  to  8 SQL queries  to  charts  to  Power BI
```

1. **Data cleaning (pandas).** Schema-agnostic column detection, currency parsing,
 and outlier handling. 3,478 raw rows to **2,838** clean rows (dropped listings
...
### 2. A ~9x geographic premium

Among neighborhoods with **≥5 listings** (to avoid single-listing noise), the most
expensive area commands roughly **9x** the rent of the cheapest.
